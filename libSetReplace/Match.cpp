...
class MatchComparator {
 ...
 public:
  MatchComparator(const Matcher::OrderingSpec& orderingSpec)
  ...
  static int compare(const MatchPtr a, const MatchPtr b,
                     const Matcher::OrderingFunction ordering) {
    switch (ordering) {
  ...
  static int compareVectors(const std::vector<ExpressionID>& first,
  ...
  static int compareSortedIDs(const MatchPtr a, const MatchPtr b,
...
class MatchEquality {
 public:
  size_t operator()(MatchPtr a, MatchPtr b) const {
    if (a->rule != b->rule ||
        a->inputExpressions.size() != b->inputExpressions.size())
      return false;
...
class Matcher::Implementation {
 private:
  ...
  // Matches are arranged in buckets. Each bucket contains matches that are
  // equivalent in terms of the ordering function, however, buckets themselves
  // are ordered according to that function. To select next match, we select a
  // random element from the first bucket. That in particular means the random
  // ordering function will automatically be used if ordering specification is
  // incomplete.

  // We use MatchPtr instead of Match to save memory, however, they are hashed
  // and sorted according to their dereferenced values in the corresponding
  // classes above.

  // We cannot directly select a random element from an unordered_map, which is
  // why we use a vector here.
  using Bucket = std::pair<
      std::unordered_map<MatchPtr, size_t, MatchHasher, MatchEquality>,
      std::vector<MatchPtr>>;
  std::map<MatchPtr, Bucket, MatchComparator> matchQueue_;
  std::unordered_map<ExpressionID,
                     std::unordered_set<MatchPtr, MatchHasher, MatchEquality>>
      expressionToMatches_;

  // A frequent operation here is detection of duplicate matches. Hashing is
  // much faster than searching for duplicates in a std::map, so we separately
  // keep a flat hash table of all matches to speed that up. That's purely an
  // optimization.
 ...
 public:
  Implementation(const std::vector<Rule>& rules, AtomsIndex& atomsIndex,
                 const std::function<AtomsVector(ExpressionID)> getAtomsVector,
                 const OrderingSpec orderingSpec, const unsigned int randomSeed)
      : rules_(rules),
  ...
  // deletion should be done in deterministic order, otherwise, the random
  // replacements will not be deterministic
 ...
 private:
  void addMatchesForRule(const std::vector<ExpressionID>& expressionIDs,
    ...
    for (size_t i = 0; i < rules_[ruleID].inputs.size(); ++i) {
      Match emptyMatch{
          ruleID, std::vector<ExpressionID>(rules_[ruleID].inputs.size(), -1)};
      completeMatchesStartingWithInput(emptyMatch, rules_[ruleID].inputs, i,
                                       expressionIDs, shouldAbort);
    }
  }

  void completeMatchesStartingWithInput(
    ...
    for (const auto expressionID : potentialExpressionIDs) {
      if (isExpressionUnused(incompleteMatch, expressionID)) {
        attemptMatchExpressionToInput(incompleteMatch, partiallyMatchedInputs,
                                      nextInputIdx, expressionID, shouldAbort);
      }
    }
  }

  static bool isExpressionUnused(const Match& match,
  ...
  void attemptMatchExpressionToInput(
      const Match& incompleteMatch,
      const std::vector<AtomsVector>& partiallyMatchedInputs,
      const size_t nextInputIdx, const ExpressionID potentialExpressionID,
      const std::function<bool()> shouldAbort) {
    ...
    if (!Matcher::substituteMissingAtomsIfPossible({input}, {expressionAtoms},
                                                   newInputs))
      return;
    ...
    const auto nextInputIdxAndCandidateExpressions =
        nextBestInputAndExpressionsToTry(newMatch, newInputs);
    if (nextInputIdxAndCandidateExpressions.first >= 0) {
      completeMatchesStartingWithInput(
          newMatch, newInputs, nextInputIdxAndCandidateExpressions.first,
          nextInputIdxAndCandidateExpressions.second, shouldAbort);
    }
  }
  ...
  void insertMatch(const Match& newMatch) {
    ...
    auto bucketIt =
        matchQueue_.find(matchPtr);  // works because comparison is smart
  ...
  void deleteMatch(const MatchPtr matchPtr) {
    ...
    for (const auto expression : expressions) {
      ...
      if (expressionToMatches_[expression].empty())
        expressionToMatches_.erase(expression);
    }
    ...
    std::swap(bucket.second[bucketIndex],
              bucket.second[bucket.second.size() - 1]);
  ...
  std::pair<size_t, std::vector<ExpressionID>> nextBestInputAndExpressionsToTry(
    ...
    // For each input, we will see how many expressions in the set contain atoms
    // appearing in this input. The fewer there are, the less branching we will
    // have to do.
    for (size_t i = 0; i < partiallyMatchedInputs.size(); ++i) {
      ...
      // For each expression, we will count how many of the input atoms appear
      // in it. We will then only use expressions that have all the required
      // atoms.
      std::unordered_map<ExpressionID, int64_t> inputAtomsCountByExpression;
      for (const auto atom : appearingAtoms) {
        for (const auto expression :
             atomsIndex_.expressionsContainingAtom(atom)) {
          inputAtomsCountByExpression[expression]++;
        }
      }

      // Here we will collect all expressions that contain all the required
      // atoms.
      ...
      // Note, if there are zero matching expressions, it means the match is not
      // possible, because none of the expressions contain all the atoms needed.
      if (nextInputIdx == -1 ||
          potentialExpressions.size() < nextExpressionsToTry.size()) {
    ...
    if (nextInputIdx == -1) {
      // We could not find any potential inputs, which means, all inputs not
      // already matched are fully patterns, and don't have any specific atom
      // references. That implies rule inputs are not a connected graph, which
      // is not supported at the moment, and would require custom logic to
  ...
  void chooseNextMatch() {
    ...
    auto distribution =
        std::uniform_int_distribution<size_t>(0, allPossibleMatches.size() - 1);
    nextMatch_ = allPossibleMatches[distribution(randomGenerator_)];
  }
};

Matcher::Matcher(const std::vector<Rule>& rules, AtomsIndex& atomsIndex,
                 const std::function<AtomsVector(ExpressionID)> getAtomsVector,
                 const OrderingSpec orderingSpec,
                 const unsigned int randomSeed) {
  implementation_ = std::make_shared<Implementation>(
      rules, atomsIndex, getAtomsVector, orderingSpec, randomSeed);
}

void Matcher::addMatchesInvolvingExpressions(
    const std::vector<ExpressionID>& expressionIDs,
    const std::function<bool()> shouldAbort) {
  implementation_->addMatchesInvolvingExpressions(expressionIDs, shouldAbort);
}

void Matcher::removeMatchesInvolvingExpressions(
    const std::vector<ExpressionID>& expressionIDs) {
...
bool Matcher::substituteMissingAtomsIfPossible(
